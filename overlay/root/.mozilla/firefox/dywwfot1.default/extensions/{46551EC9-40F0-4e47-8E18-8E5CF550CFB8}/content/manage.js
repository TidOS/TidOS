var stylishManage = {
	...
	updateAll: function() {
		//doing them all at once overwhelms the site
		var service = Components.classes["@userstyles.org/style;1"].getService(Components.interfaces.stylishStyle);
		stylishManage.currentUpdateStyles = service.list(service.REGISTER_STYLE_ON_CHANGE, {});
		stylishManage.currentUpdateIndex = 0;
		stylishManage.updateNext();
	},

	updateNext: function() {
		var i = stylishManage.currentUpdateIndex;
		stylishManage.currentUpdateIndex++;
		if (i >= stylishManage.currentUpdateStyles.length) {
			//we're done
			return;
		}
		stylishManage.currentUpdateStyles[i].checkForUpdates();
	},*/
