...
// @description  Intuitive 4chan GUI for fast replies + Forwardlinks/Backlinks snapshots and nesting + Filters with and different filter actions + View all attached images + Auto-threading + more.
// @version        1.3.0
...
if(browser=='Chrome'){
  ...
  //PASTE GM FUNCTION EMULATORS IMMEDIATELY BELOW
...
String.prototype.isOneOfThese = function(csv){ //String IsOneOfThese - By Jenseki Mora
	var a= new Array(); a= csv.split(','); var al= a.length;
  for(var fi=0;fi<al;fi++){
    if(this==a[fi]) return true;
  }
  return false;
}
String.prototype.isNoneOfThese = function(csv){ //String IsNoneOfThese - By Jenseki Mora
	var a= new Array(); a= csv.split(','); var al= a.length;
  for(var fi=0;fi<al;fi++){
    if(this==a[fi]) return false;
  }
  return true;
}
function uriGet(querry){                        //URL guery parameter fetcher
  var bar, out, q, q2, el, e2;
	bar=document.location.href; out=''; q=bar.indexOf('\?'); e1=bar.indexOf('=',q+1);
	if(q>-1&&bar.substr(q+1,e1-q-1)==querry) { out=uriGet2(bar,e1+1); }
	else if(q>-1){
...
function setPrefs(){                             //sets user preferences
  ...
  GM_setValue('repfilter',document.getElementById('cci-post-filters').value);
  /*GM_setValue('customcss',document.getElementById('cci-customcss').value);*/
  ...
  if(isNaN(parseFloat(temp))||parseFloat(temp)<0||parseFloat(temp)>1)
    temp='1.00';
...
function addForwardLink(qid,tid){                //ads a forwardlink of a quoting reply to a post
  ...
  if(bq!=undefined){
    ...
    bqr.innerHTML=bqr.innerHTML+'<br><font><a href="javascript: void 0;" onclick="chanchimp_placequoted(this,\''+tid+'\');" onmouseover="chanchimp_showquoted(\''+tid+'\');" onmouseout="chanchimp_hidequoted();" class="quotelink-forwardlink">&gt;&gt;'+tid+'</a></font>';
  }
}
function linkify(text){
    if (text)
      text = text.replace(/((https?:\/\/|ftp:\/\/)([A-Za-z0-9_:#%$!;,?&=@~"'\/\\\.\-\+\?\[\] ]*(.html?)|[A-Za-z0-9_:#%$!;,?&=@~"'\/\\\.\-\+\?\[\]]*))/g, "<u><a target='_blank' href='$1'>$1</a></u>");
    return text;
}
function linkifyAll(){
...
function inScope(meta){                          //checks if a filter is within scope
  ...
  pos1=meta.indexOf('scope:')+6; pos2=meta.indexOf(';',pos1); scope= meta.substr(pos1,pos2-pos1).trim(); res=true;
  ...
  for(;proceed==1;){ //loop until proceed becomes zero
    if(notNull(cond)&&cond.match(/(@)(postnumber|name|tripcode|nametripcode|email|filename|filesize|imagewidth|imageheight|comment|commenthtml|(out)[0-9]+)(\->)/)==null) //if there are no more condition rules
      ...
      if(notNull(input)&&notNull(func)&&notNull(param)){
        try{ //IMPORTANT
          if(func.indexOf('CS')>-1) {func= func.substr(0,func.length-2); mode1= 'CS';}
          else if(func.indexOf('RE')>-1) {func= func.substr(0,func.length-2); mode1= 'RE';}
          if(func=='hasText'||func=='exactText'||func=='startsWith'||func=='endsWith'){
            if(mode1!='CS'&&mode1!='RE')
              mode1='DE';
            if(eval('f_check_'+func+'(\''+input+'\',\''+fi+'\',\''+param+'\',\''+mode1+'\')')) res= true;
            else res= false;
          }
          if(func.indexOf('value')===0) {mode1= 'value';}
          else if(func.indexOf('length')===0) {mode1= 'length';}
          if(func.indexOf('GTE')>-1) {func= func.substr(0,func.length-3); mode2= 'GTE';}
          else if(func.indexOf('GT')>-1) {func= func.substr(0,func.length-2); mode2= 'GT';}
          else if(func.indexOf('LTE')>-1) {func= func.substr(0,func.length-3); mode2= 'LTE';}
          else if(func.indexOf('LT')>-1) {func= func.substr(0,func.length-2); mode2= 'LT';}
...
function selectInput(call,fi){                  //returns corresponding value in the postinfo array of the @data in post index fi
  //global var postinfo;
  switch(call){
    case 'postnumber': return postinfo[fi][0]; break;
    case 'name': return postinfo[fi][1]; break;
    case 'tripcode': return postinfo[fi][2]; break;
    case 'nametripcode': return postinfo[fi][1]+' '+postinfo[fi][2];
    case 'email': return postinfo[fi][3]; break;
    case 'filename': return postinfo[fi][4]; break;
    case 'filesize': return postinfo[fi][5]; break;
    case 'imagewidth': return postinfo[fi][6]; break;
    case 'imageheight': return postinfo[fi][7]; break;
    case 'comment': return postinfo[fi][8].replace(/(<)([a-zA-Z\/])([a-zA-Z0-9_ \\\/"'\.\-\+():;,={}]*)(>)/ig,''); break; //do not pass html tag character sequences
    case 'commenthtml': return postinfo[fi][8];
    default: 
...
function doFilter(meta,fi){                     //render the proper filter action
  ...
  pos1=meta.indexOf('id:')+3; pos2=meta.indexOf(';',pos1); id= meta.substr(pos1,pos2-pos1).trim();
  obj=document.getElementById(postinfo[fi][0]); pos1=meta.indexOf('action:')+7; pos2=meta.indexOf(';',pos1);
  action= meta.substr(pos1,pos2-pos1).trim();
  if(action=='hide'){
    obj.innerHTML='<div id="filter-hidden-'+postinfo[fi][0]+'" style="display: none;">'+obj.innerHTML+'</div>'
    +'<div id="filter-note-'+postinfo[fi][0]+'"><small>ID:: '+id+' <a href="javascript: void 0;" onclick="chanchimp_showFiltered(\''+postinfo[fi][0]+'\');" style="text-decoration: none;">[Show]</a></small></div>';
  }
    ...
    obj.innerHTML='<div><small>ID:: '+id+' <a href="javascript: void 0;" onclick="chanchimp_enlargeFiltered(\''+postinfo[fi][0]+'\');" style="text-decoration: none;">[Enlarge]</a></small></div>'
    +'<div style="font-size: 10px;">'+obj.innerHTML+'</div>';
  }
}
function escapeRegExp(param,mode){
  var str;
  switch(mode){
    case 'DE': str='/'+param.replace(/(\.|\?|\[|\]|\(|\)|\+|\-|\{|\}|\*|\/|\\)/gi,'\\\$1')+'/i'; break;
    case 'CS': str='/'+param.replace(/(\.|\?|\[|\]|\(|\)|\+|\-|\{|\}|\*|\/|\\)/gi,'\\\$1')+'/'; break;
    case 'RE': str=param; break;
  }
  return str;
}
...
function f_check_compare(inputtxt,fi,param,mode1,mode2){
  ...
  if(input===false) return false;
  switch(mode1){
    case 'value': input2= parseInt(input); break;
    case 'length': input2= input.length; break;
    default:
  }
  switch(mode2){
...
function checkSinglequote(id){                              //checks if a post is quoting one post only
  var obj;
  ...
  if(ql[0]!=undefined&&ql[1]==undefined) return true;
  else return false;
}
function movePost(id){                                      //moves a post under thepost it replies to
  var obj, tid;
  obj=document.getElementById(id);
  tid= obj.querySelectorAll('a.quotelink')[0].innerHTML.substr(8);
  putUnder(tid,obj);
}
function putUnder(id,obj){                                  //sunfunction of movePost()
  var quotedobj, this_level, temp, i;
  if(document.getElementById('nothread'+id)!=undefined){//if quoted post is original post
    quotedobj='op'; this_level=2;
  }
...
function crawlAndPut(obj,this_level,currentunderobj){      //subfunction of crawlAndPut
  ...
  if(currentunderobj=='op'){
    ...
    if(obj==nextObj){
      if(repliesNode.getElementsByTagName('table')[1]!=undefined)
        nextObj= repliesNode.getElementsByTagName('table')[1];
      else
        nextObj= '';
    }
  }
...
function nextTableElement(obj){                  //returns the next table element after obj
  var out, i;
  out= false; parentOfElement= obj.parentNode;
  var elementarray= new Array();
  elementarray= parentOfElement.getElementsByTagName('table');
  var el= elementarray.length;
  for(i=0;i<el;i++){
    if(obj==elementarray[i]&&elementarray[i+1]!=undefined)
      out= elementarray[i+1];
  }
  return out;
}
...
function addQuoteFeatures(quotenums,quotelinks){
  ...
  for(i=0;i<l;i++){
    if(parseInt(quotenums[i].innerHTML)>100){
      ...
      quotenums[i].setAttribute('onclick','chanchimp_showPostArea(); chanchimp_quote(\''+quotenums[i].innerHTML+'\');');
    }
  }
  if(GM_getValue('cci_quotesnapshot',true)){
    ...
    for(i=0;i<l;i++){
      ...
      quotelinks[i].setAttribute('onmouseover','chanchimp_showquoted(\''+qid+'\');');
      quotelinks[i].setAttribute('onmouseout','chanchimp_hidequoted();');
      quotelinks[i].setAttribute('onclick','chanchimp_placequoted(this,\''+qid+'\');');
...
function processInfo(tdobj,fk){
  ...
  //name
  if(tdobj.querySelectorAll('span.commentpostername')[0]!=undefined)
    postinfo[fk][1]= tdobj.querySelectorAll('span.commentpostername')[0].innerHTML.replace(/((<a|<\/)([a-zA-Z0-9_ =\-\+\/\\\."%;,:]*)(>))|(Anonymous)/g,'');
  //tripcode
  if(tdobj.querySelectorAll('span.postertrip')[0]!=undefined)
    postinfo[fk][2]= tdobj.querySelectorAll('span.postertrip')[0].innerHTML;
  //email
  if(tdobj.querySelectorAll('span.linkmail')[0]!=undefined)
    postinfo[fk][3]= tdobj.querySelectorAll('a.linkmail')[0].href.substr(7);
  //filename
  if(tdobj.querySelectorAll('span.filesize')[0]!=undefined&&tdobj.querySelectorAll('span.filesize')[0].getElementsByTagName('span')[0]!=undefined)
    postinfo[fk][4]= tdobj.querySelectorAll('span.filesize')[0].getElementsByTagName('span')[0].innerHTML;
  ...
  if(tdobj.getElementsByTagName('blockquote')[0]!=undefined)
    postinfo[fk][8]=tdobj.getElementsByTagName('blockquote')[0].innerHTML.replace(/(<div class="forwardlinks">).*(<\/div>)/i,'');
  for(j=0;j<9;j++){
    if(postinfo[fk][j]==undefined)
      postinfo[fk][j]='';
    else
      postinfo[fk][j]= postinfo[fk][j].trim();
  }
}
...
switch(GM_getValue('cci_postarea_color','red')){
  case 'red': colorscheme[0]='rgba(224,64,0,'+opacity+')'; colorscheme[1]='rgba(240,224,214,'+opacity+')'; colorscheme[2]='rgba(243,239,235,'+opacity+')'; break;
  case 'orange': colorscheme[0]='rgba(255,113,0,'+opacity+')'; colorscheme[1]='rgba(251,231,128,'+opacity+')'; colorscheme[2]='rgba(255,246,168,'+opacity+')'; break;
  case 'yellow': colorscheme[0]='rgba(255,255,0,'+opacity+')'; colorscheme[1]='rgba(255,255,183,'+opacity+')'; colorscheme[2]='rgba(255,255,204,'+opacity+')'; break;
  case 'green': colorscheme[0]='rgba(0,195,34,'+opacity+')';colorscheme[1]='rgba(214,240,214,'+opacity+')'; colorscheme[2]='rgba(235,243,235,'+opacity+')'; break;
  case 'blue': colorscheme[0]='rgba(73,101,214,'+opacity+')'; colorscheme[1]='rgba(214,218,240,'+opacity+')'; colorscheme[2]='rgba(232,235,252,'+opacity+')'; break;
  case 'purple': colorscheme[0]='rgba(153,50,204,'+opacity+')'; colorscheme[1]='rgba(220,208,255,'+opacity+')'; colorscheme[2]='rgba(235,228,255,'+opacity+')'; break;
  case 'gray': colorscheme[0]='rgba(128,128,128,'+opacity+')'; colorscheme[1]='rgba(221,221,221,'+opacity+')'; colorscheme[2]='rgba(236,236,236,'+opacity+')'; break;
  default:
}
...
if(cond_isboard===true||cond_isthread===true)
  cond_isthreadorboard=true;
...
for(i= 0;i<rl;i++){
  ...
  if(postarray_t[i].id=='')
    postarray_t[i].setAttribute('id','cci_4chan_reply_'+postarray[i].id);
}
...
if(cond_isthreadorboard && cond_notccvai){
  ...
  var field_size= Array(); var table_width= Array(); var shortwidth=0;
  field_size[0]=200; field_size[1]=80; field_size[2]=35; field_size[3]=10; field_size[4]=700; field_size[5]=490; table_width[0]=800; table_width[1]=500; table_width[2]=300;
  if(document.body.clientWidth<1000){
    field_size[0]=150; field_size[1]=40; field_size[2]=30; field_size[3]=5; field_size[4]=630; field_size[5]=420; table_width[0]=700; table_width[1]=440; table_width[2]=260;
  }
...
Template D\n';
  ...
  try{
    ...
    for(i=0; i<post_templates.length; i++){
      ...
      if(i!=post_templates.length-2)
        temp2+=",";
      i++;
    }
    temp="\
      function chanchimp_tempquoted(id){\
        var temp= Array();\
      ...
      function chanchimp_showquoted(id){\
        var cont=document.getElementById('cc-quotedcontainer');\
        var temp= new Array();\
        temp=chanchimp_tempquoted(id);\
      ...
      function chanchimp_placequoted(obj,id){\
        chanchimp_hidequoted();\
        ...
        obj.setAttribute('onclick','chanchimp_unplacequoted(this,\\''+id+'\\');');\
      ...
      function chanchimp_unplacequoted(obj,id){\
        obj.setAttribute('onclick','chanchimp_placequoted(this,\\''+id+'\\');');\
        obj.setAttribute('onmouseover','chanchimp_showquoted(\\''+id+'\\');');\
        var objparent=obj.parentNode;\
        var p= objparent.getElementsByTagName('table')[0];\
        objparent.removeChild(p);\
      }\
      function chanchimp_getMouseXY(e) {\
        cc_posX = e.pageX;\
        cc_posY = e.pageY;\
        ...
        if(cc_beingDragged===true&&cc_dragId!=''){\
          ...
          if(cc_dragXOffset==-1)\
            cc_dragXOffset=cc_posX-chanchimp_getObjPosX(d);\
          d.style.left=cc_posX-cc_dragXOffset+'px';\
          d.style.top=cc_posYf-4+'px';\
        }\
      }\
      function chanchimp_getObjPosX(obj){\
      ...
      function chanchimp_dragBlock(id){\
        cc_dragId=id; cc_beingDragged= true;\
      }\
      function chanchimp_releaseDrag(){\
        cc_dragId=''; cc_beingDragged= false; cc_dragXOffset=-1;\
      }\
      function chanchimp_handleKey(e){\
      	if(e.which) keycode=e.which;\
        else keycode = e.keyCode;\
        switch(keycode){\
          case 27: chanchimp_togglePostArea(); break;\
      		case 45: chanchimp_toggleTransform(); break;\
      		default:\
      	}\
      }\
      function chanchimp_togglePostArea(){\
      ...
      function chanchimp_showPostArea(){\
        if(document.getElementById('cc-showpostarea-toggle').innerHTML=='Show'){\
          chanchimp_togglePostArea();\
        }\
      }\
      function chanchimp_forceHidePostArea(){\
          document.getElementById('cc-showpostarea-toggle').innerHTML='Show';\
          document.getElementById('cc-newpostarea').style.display='none';\
          document.getElementById('cc-newpostarea2').style.display='none';\
      }\
      function chanchimp_fieldsToggle(){\
      ...
      function chanchimp_picsurlsToggle(){\
      ...
      function chanchimp_templatesMenuToggle(){\
      ...
      function chanchimp_postTemplate(i){\
        ...
        chanchimp_templatesMenuToggle();\
        document.getElementsByName('com')[0].focus();\
      }\
      function chanchimp_quote(id){\
        var txt= chanchimp_getSelText('quoteclick');\
      ...
      function chanchimp_quoteInBoard(thread_id,id){\
        var obj= document.getElementsByName('com')[0];\
      ...
      function chanchimp_getSelText(mode){\
        var txt = '';\
      ...
      function chanchimp_toggleTransform(){\
        if(cc_pfamode=='floater')\
          chanchimp_transformToPopUp();\
        else \
          chanchimp_transformToFloater();\
        chanchimp_showPostArea();\
      }\
      function chanchimp_transformToPopUp(){\
        chanchimp_forceHidePostArea();\
        var form= document.getElementById('post');\
        var form2= document.getElementById('post2');\
        for(var i=0; form.getElementsByTagName('input')[i]!=undefined; i++){\
          if(form.getElementsByTagName('input')[i].type!='submit'&&form.getElementsByTagName('input')[i].id!='cci-newthread'){\
            form.getElementsByTagName('input')[i].name= form.getElementsByTagName('input')[i].name+'3';\
            form2.getElementsByTagName('input')[i].name= form2.getElementsByTagName('input')[i].name.substr(0,form2.getElementsByTagName('input')[i].name.length-1);\
            if(form.getElementsByTagName('input')[i].type!='file')\
              form2.getElementsByTagName('input')[i].value= form.getElementsByTagName('input')[i].value;\
          }\
        }\
        ...
        document.getElementById('cci-newthread-popup').appendChild(document.getElementById('cci-newthread'));\
        cc_pfamode='popup';\
      }\
      function chanchimp_transformToFloater(){\
        chanchimp_forceHidePostArea();\
        ...
        for(var i=0; form2.getElementsByTagName('input')[i]!=undefined; i++){\
          if(form2.getElementsByTagName('input')[i].type!='submit'&&form2.getElementsByTagName('input')[i].id!='cci-newthread'){\
            form2.getElementsByTagName('input')[i].name= form2.getElementsByTagName('input')[i].name+'2';\
            form.getElementsByTagName('input')[i].name= form.getElementsByTagName('input')[i].name.substr(0,form.getElementsByTagName('input')[i].name.length-1);\
            if(form2.getElementsByTagName('input')[i].type!='file')\
              form.getElementsByTagName('input')[i].value= form2.getElementsByTagName('input')[i].value;\
          }\
        }\
        ...
        document.getElementById('cci-newthread-floater').appendChild(document.getElementById('cci-newthread'));\
        cc_pfamode='floater';\
      }\
      function chanchimp_submitForm(isthread){\
        var formobj= document.getElementById('post');\
        var obj= formobj.getElementsByTagName('textarea')[0];\
        var id;\
        var proceed=1;\
        if(document.getElementById('cci-newthread').checked){\
      ...
      function chanchimp_pfafocus(num){\
        cc_pfafocus++;\
      ...
      function chanchimp_pfablur(num){\
        cc_pfafocus--;\
      ...
      var cc_pfafocus= 0;\
      document.onmousemove = chanchimp_getMouseXY;\
      document.onkeydown = chanchimp_handleKey;\
    ";
  	e.innerHTML= temp;
  	head.appendChild(e);
  }catch(e){}
  ...
  if(web.isNoneOfThese('4chanarchive')){
      ...
      <table id="cci-form-container" style="width: 100%; display: ;"><tr>\
        <td id="cc-fields2" style="width: 0px;">\
          <table style="cell-spacing: 2px; display: none;">\
            <tr><td><b>Subject:</b></td><td><input class=inputtext type=text name=sub style="width: '+field_size[0]+'px" onfocus="chanchimp_pfafocus(1);" onblur="chanchimp_pfablur(1);"></td></tr>\
            <tr><td><b>Name:</b></td><td><input class=inputtext type=text name=name style="width: '+field_size[0]+'px" onfocus="chanchimp_pfafocus(1);" onblur="chanchimp_pfablur(1);"></td></tr>\
            <tr><td><b>Email:</b></td><td><input class=inputtext type=text name=email style="width: '+field_size[0]+'px" onfocus="chanchimp_pfafocus(1);" onblur="chanchimp_pfablur(1);"></td></tr>\
            <tr><td><b>Password:</b></td><td><input class=inputtext type=password name="pwd" style="width: '+field_size[1]+'px" maxlength=8 value="" onfocus="chanchimp_pfafocus(1);" onblur="chanchimp_pfablur(1);"> [<label><input type=checkbox name=spoiler value=on>Spoiler Image?</label>]</td></tr>\
          </table>\
        </td><td id="cc-fields1" style="width: '+table_width[0]+'px;">\
            <a href="javascript: void 0;" onclick="chanchimp_fieldsToggle();" id="togglefieldstxt" style="text-decoration: none;">◄</a> \
            <b>File:</b><input type=file name=upfile size='+field_size[2]+'> <input type=button value="Submit" onclick="chanchimp_submitForm('+cond_isthread+');"> <input type=button value="Noko" onclick="document.post.email.value=\'noko\'; chanchimp_submitForm('+cond_isthread+');"> <input type=button value="Sage" onclick="document.post.email.value=\'sage\'; chanchimp_submitForm('+cond_isthread+');">\
            <br/><b>Comment:</b> <a href="javascript: void 0;" style="text-decoration: none;" onclick="chanchimp_templatesMenuToggle();">[Shortcuts]</a> <a href="javascript: void 0;" style="text-decoration: none;" onclick="chanchimp_spoiler();">[Spoiler]</a> <a href="javascript: void 0;" style="text-decoration: none;" onclick="chanchimp_getSelText(\'\');">[Selected]</a> <a href="javascript: void 0;" style="text-decoration: none;" onclick="document.getElementsByName(\'com\')[0].value=\'\'; document.getElementsByName(\'com\')[0].focus();">[Reset]</a> <span id="cci-newthread-floater"><input type=checkbox id="cci-newthread" /></span> New thread\
            <br/><textarea class=inputtext name=com style="width: '+field_size[4]+'px" rows="4" wrap=soft onfocus="chanchimp_pfafocus(1);" onblur="chanchimp_pfablur(1);"></textarea>\
        </td><td style="vertical-align: top;">\
          <table><tr>\
          <td>\
            <p><a style="text-decoration: none;" href="javascript: void 0;" onclick="window.scrollTo(0,0);" title="Go to top">▲</a> | <a style="text-decoration: none;" href="javascript: void 0;" onclick="window.scrollTo(0,1000000);" title="Go to bottom">▼</a> | <a style="text-decoration: none;" href="javascript: void 0;" onclick="chanchimp_gotoToggle();">GoTo</a>\
            '+(cond_isthread?'<br/>All images: <a style="text-decoration: none;" target="_blank" href="'+document.location.href+'?&chanchimp=on&chanchimp_n=30&chanchimp_p=1">View</a> | <a style="text-decoration: none;" href="javascript: void 0;" onclick="chanchimp_picsurlsToggle();">Links</a>':'')+'\
            '+(cond_isthread?'<br/><a style="text-decoration: none;" target="_blank" href="http://4chanarchive.org/brchive/main.php?mode=submit&chanchimp_id='+thread_id+'&chanchimp_board='+board+'">Submit to 4chanarchive</a>':'')+'\
            <br/><a style="text-decoration: none;" href="javascript: void 0;" onclick="chanchimp_prefToggle();">Preferences</a>\
            <p><b><a id="updatetext" style="text-decoration: none;" href="http://sites.google.com/site/netroscripts/chanchimp">ChanChimp</a></b>\
            </td>\
          <td>\
    ...
    <div style="padding: 4px; width: 100%; border-top: solid 2px '+colorscheme[0]+';">\
      ...
      '+(cond_isthread?'| (All images: <a style="text-decoration: none;" target="_blank" href="'+document.location.href+'?&chanchimp=on&chanchimp_n=30&chanchimp_p=1">View</a> | <a style="text-decoration: none;" href="javascript: void 0;" onclick="chanchimp_picsurlsToggle();">Links</a>)':'')+'\
      | <a style="text-decoration: none;" href="javascript: void 0;" onclick="chanchimp_prefToggle();">Preferences</a>\
      | <b><a id="updatetext" style="text-decoration: none;" href="http://sites.google.com/site/netroscripts/chanchimp">ChanChimp</a></b> | <small> <a style="text-decoration: none;" href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=10158704">Donate</a> </small>\
    </div>\
    ';
  }
  try{
    ...
    e.style.position = 'fixed';
    e.style.width = '100%';
    e.style.height = (web.isOneOfThese('4chanarchive')?'30px':'150px');
    e.style.left = '0px';
    e.style.bottom= '0px';
    ...
    e.setAttribute('class','chanchimp');
    if(web.isOneOfThese('4chan')){
      e.setAttribute('onmouseover','chanchimp_pfafocus(1)');
      e.setAttribute('onmouseout','chanchimp_pfablur(1)');
    }
  }catch(e){}
  
  if(web.isNoneOfThese('4chanarchive')){
    temp='\
    <div id="cc-dragpopup" style="width: 100%; font-size: 8px; background-color: '+colorscheme[0]+'; cursor: move;" onmousedown="chanchimp_dragBlock(\'cc-newpostarea2\');" onmouseup="chanchimp_releaseDrag();">\
      <a href="javascript: void 0;" onclick="chanchimp_fieldsToggle();" id="togglefieldstxt2" style="text-decoration: none; color: white;">◄</a>&nbsp;&nbsp;\
      <a href="javascript: void 0;" onclick="chanchimp_togglePostArea();" style="text-decoration: none; color: white;"><b>—</b></a> \
    </div>\
    <div style="padding: 4px; width: 100%;">\
    <form name=post2 id="post2" action="'+form1_action+'" method="POST" enctype="multipart/form-data" target="" style="display: inline;">\
      ...
      <tr>\
        <td id="cc-fields2-2">\
          <table style="cell-spacing: 2px; display: none;">\
            <tr><td><b>Subject:</b></td><td><input class=inputtext type=text name=sub2 style="width: '+field_size[0]+'px" onfocus="chanchimp_pfafocus(2);" onblur="chanchimp_pfablur(2);"></td></tr>\
            <tr><td><b>Name:</b></td><td><input class=inputtext type=text name=name2 style="width: '+field_size[0]+'px" onfocus="chanchimp_pfafocus(2);" onblur="chanchimp_pfablur(2);"></td></tr>\
          </table>\
        </td>\
        <td id="cc-fields3-2">\
          <table style="cell-spacing: 2px; display: none;">\
            <tr><td><b>Email:</b></td><td><input class=inputtext type=text name=email2 style="width: '+field_size[0]+'px" onfocus="chanchimp_pfafocus(2);" onblur="chanchimp_pfablur(2);"></td></tr>\
            <tr><td><b>Password:</b></td><td><input class=inputtext type=password name="pwd2" style="width: '+field_size[1]+'px" maxlength=8 value="" onfocus="chanchimp_pfafocus(2);" onblur="chanchimp_pfablur(2);"> [<label><input type=checkbox name=spoiler value=on>Spoiler Image?</label>]</td></tr>\
          </table>\
        </td>\
      </tr>\
      <tr>\
        <td id="cc-fields1-2" style="width: '+table_width[0]+'px;" colspan=2>\
            <b>File:</b><input type=file name=upfile2 size='+field_size[2]+'> <input type=button value="Submit" onclick="chanchimp_submitForm('+cond_isthread+');"> <input type=button value="Noko" onclick="document.post.email.value=\'noko\'; chanchimp_submitForm('+cond_isthread+');"> <input type=button value="Sage" onclick="document.post.email.value=\'sage\'; chanchimp_submitForm('+cond_isthread+');">\
            <br/><b>Comment:</b> <a href="javascript: void 0;" style="text-decoration: none;" onclick="chanchimp_templatesMenuToggle();">[Shortcuts]</a> <a href="javascript: void 0;" style="text-decoration: none;" onclick="chanchimp_spoiler();">[Spoiler]</a> <a href="javascript: void 0;" style="text-decoration: none;" onclick="chanchimp_getSelText(\'\');">[Selected]</a> <a href="javascript: void 0;" style="text-decoration: none;" onclick="document.getElementsByName(\'com\')[0].value=\'\'; document.getElementsByName(\'com\')[0].focus();">[Reset]</a> <a id="cc-commentfieldtoggle" href="javascript: void 0;" style="text-decoration: none;" onclick="toggleCommentField();">[Expand]</a> <span id="cci-newthread-popup"></span> New Thread\
            <br/><textarea class=inputtext name=com2 rows="4" wrap=soft style="width: 575px;" onfocus="chanchimp_pfafocus(2);" onblur="chanchimp_pfablur(2);"></textarea>\
        </td>\
      </tr>\
      <tr>\
        <td style="vertical-align: top;" colspan=2>\
            <a style="text-decoration: none;" href="javascript: void 0;" onclick="window.scrollTo(0,0);" title="Go to top">▲</a> | <a style="text-decoration: none;" href="javascript: void 0;" onclick="window.scrollTo(0,1000000);" title="Go to bottom">▼</a> | <a style="text-decoration: none;" href="javascript: void 0;" onclick="chanchimp_gotoToggle();">GoTo</a>\
            '+(cond_isthread?'| (All images: <a style="text-decoration: none;" target="_blank" href="'+document.location.href+'?&chanchimp=on&chanchimp_n=30&chanchimp_p=1">View</a> | <a style="text-decoration: none;" href="javascript: void 0;" onclick="chanchimp_picsurlsToggle();">Links</a>)':'')+'\
            '+(cond_isthread?'| <a style="text-decoration: none;" target="_blank" href="http://4chanarchive.org/brchive/main.php?mode=submit&chanchimp_id='+thread_id+'&chanchimp_board='+board+'">Submit to 4chanarchive</a>':'')+'\
            | <a style="text-decoration: none;" href="javascript: void 0;" onclick="chanchimp_prefToggle();">Preferences</a>\
            | <b><a id="updatetext2" style="text-decoration: none;" href="http://sites.google.com/site/netroscripts/chanchimp">ChanChimp</a></b>\
        </td>\
      </tr>\
      </table>\
    </form>\
    ...
    try{
      ...
      body.appendChild(e);
      e.setAttribute('class','chanchimp');
      e.setAttribute('onmouseover','chanchimp_pfafocus(2)');
      e.setAttribute('onmouseout','chanchimp_pfablur(2)');
    }catch(e){}
  ...
  try{
    ...
    for(i=0;i<post_templates.length;i+=2)
      temp+='<div onclick="chanchimp_postTemplate('+i/2+');" style="cursor: pointer;" onmouseover="this.style.backgroundColor= \''+colorscheme[0]+'\';" onmouseout="this.style.backgroundColor= \'\';">'+post_templates[i]+'</div>';
    e.innerHTML = temp;
    body.appendChild(e);
    e.setAttribute('class','chanchimp');
  }catch(e){}
  ...
  if(cond_isthread){
    ...
    if(GM_getValue('cci_autothread',false)===true){
      ...
      for(i=0;i<replies_al;i++){ //remove <a> tags
        if(replies_a[i]!=undefined&&replies_a[i].parentNode==repliesdiv)
          repliesdiv.removeChild(replies_a[i]);
      }
      ...
      for(i=0;i<rl;i++){
        if(checkSinglequote(postarray_t_ids[i]))
          movePost(postarray_t_ids[i]);
      }
      if(web.isNoneOfThese('4chanarchive')){
    ...
    j=1;
    for(i=0;i<rl;i++){
      if(postarray[i].querySelectorAll('span.filesize')[0]!=undefined&&postarray[i].querySelectorAll('span.filesize')[0].getElementsByTagName('a')[0]!=undefined&&postarray[i].querySelectorAll('span.filesize')[0].getElementsByTagName('a')[0].href!='')
        picsurls[j++]=postarray[i].querySelectorAll('span.filesize')[0].getElementsByTagName('a')[0].href;
    }
    temp='<div><div style="float: left;"><b>Image URLs:</b></div><div style="float: right"><a style="text-decoration: none;" href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=10158704" target="_blank">Donate</a> | <a style="text-decoration: none;" href="javascript: void 0;"  onclick="chanchimp_picsurlsToggle();">Close</a></div><div style="clear: both;"></div></div><textarea style="width:580px; height: 350px;" wrap="off">';
    l=picsurls.length;
    for(i=0;i<l;i++){
      temp+=picsurls[i]+"\n";
    }
  ...
  if(cond_isboard && web.isNoneOfThese('4chanarchive')){
    ...
    for(i=0;i<l;i++){
      if(parseInt(quotenums[i].innerHTML)>100){
        temp= quotenums[i].href.substr(quotenums[i].href.indexOf('res/')+4,quotenums[i].href.indexOf('#')-quotenums[i].href.indexOf('res/')-4);
        quotenums[i].setAttribute('href','javascript: void 0;');
        quotenums[i].setAttribute('onclick','chanchimp_showPostArea(); chanchimp_quoteInBoard(\''+temp+'\',\''+quotenums[i].innerHTML+'\');');
      }
    }
  }
  ...
  temp='\
    <div style="width: 700px; padding: 10px; background-color: '+colorscheme[2]+'; color: black; border: solid 3px '+colorscheme[0]+'; -moz-border-radius: 5px; -webkit-border-radius: 5px;">\
     <div style="border-bottom: solid 1px '+colorscheme[0]+'; padding-bottom: 3px;">\
        ...
        <div style="float: right;"><a style="text-decoration: none;" href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=10158704" target="_blank">Donate</a> | <a style="text-decoration: none;" href="javascript: void 0;"  onclick="chanchimp_prefToggle();">Close</a></div>\
  ...
  try{
    ...
    temp="";
    if(web.isNoneOfThese('4chanarchive'))
      temp+="document.post.name.value=get_cookie('4chan_name'); document.post.email.value=get_cookie('4chan_email'); document.post.pwd.value=get_pass('4chan_pass');";
    if(web.isNoneOfThese('4chanarchive')&&(GM_getValue('cci_postarea_popup',false)===true||shortwidth==1)) temp+=" chanchimp_transformToPopUp();";
    if(GM_getValue('cci_postarea_hidden',true)===false) temp+=" chanchimp_showPostArea();";
    if(GM_getValue('cci_postarea_twofields',true)===false && web.isNoneOfThese('4chanarchive')) temp+=" chanchimp_fieldsToggle();";
  ...
  if((last_checkupdate+86400000)<timenow){
    if(Math.random()<=0.1)
      GM_setValue('lastcheckupdate',timenow+'');
    try{
      e= document.createElement('script');
      e.type= "text/javascript";
...
if(uri.indexOf('http://4chanarchive.org/brchive/main.php?mode=submit')>-1){
  try{
  	...
  	e.type= "text/javascript";
    temp="\
      document.getElementsByTagName('form')[0].getElementsByTagName('input')[2].value='"+uriGet('chanchimp_id')+"';\
      document.getElementsByTagName('form')[0].getElementsByTagName('select')[0].value='"+uriGet('chanchimp_board').toUpperCase()+"';\
    ";
  	e.innerHTML= temp;
  	body.appendChild(e);
  }catch(e){}
...
if(cond_isthread && !cond_notccvai){
  ...
  try{
    ...
    e.type= "text/javascript";
    temp="\
    function chanchimp_go(){\
      var out='?&chanchimp=on';\
      out+='&chanchimp_n='+document.getElementById('chanchimp-select-n').options[document.getElementById('chanchimp-select-n').selectedIndex].value;\
      out+='&chanchimp_p=1';\
      document.location=document.location.href.substr(0,document.location.href.indexOf('?&chanchimp=on'))+out;\
    }\
    function chanchimp_go_page(str){\
      ...
      var p= parseInt(appendloc.substr(appendloc.indexOf('&chanchimp_p=')+13));\
      var out= appendloc.substr(0,appendloc.indexOf('&chanchimp_p='))+'&chanchimp_p='+(str=='next'?p+1:p-1);\
      if(str!='back')\
  ...
  for(i=0;document.getElementsByTagName('a')[i]!=undefined;i++){
    ...
    if(document.getElementsByTagName('a')[i]){
      if(
        ...
        if(obj.href!=undefined&&obj.href!=''&&checkDupe(obj.href)===false)
          pics[j++]=obj.href;
      }
    }
  }
  ...
  var p=parseInt(uriGet('chanchimp_p'));
  var n=parseInt(uriGet('chanchimp_n'));
  var o = n;
  if(pics.length<p*n)
    o=pics.length-(p-1)*n;
  ...
  if(i==(p-1)*n){
    ...
    temp+='<br/><br/>'+(p==1?'':'<input type="submit" value="Previous Page" onclick="chanchimp_go_page(\'prev\');" />')+(pics.length>p*n?'<input type="submit" value="Next Page" onclick="chanchimp_go_page(\'next\');" />':'');
  }
...
// ---------------------------------------------- DOM INSERTED EVENTS ---------------------------------------------------------------------
  ...
  if(web.isOneOfThese('4chan')&&cond_isthread){
    document.body.addEventListener('DOMNodeInserted',function(e){processInsertedDOM(e)},true);
  }
