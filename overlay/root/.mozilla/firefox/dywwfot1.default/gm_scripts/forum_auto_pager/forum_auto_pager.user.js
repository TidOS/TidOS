...
//
// Version 0.5.9:
// script update to avoid opera script error
...
var vBulletin3_8 = function(){
	...
	that.init = function(){
		...
		var a=xpath.snapshotItem(0).parentNode.getElementsByTagName('a');a=a[a.length-1].href.toString();
		var result=null;
	...
	that.ProcessResultData = function(data, obj){
		...
		xpath2 = document.evaluate("./ancestor::table"
			,xpath.snapshotItem(1),null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
		content[1]=xpath2.snapshotItem(xpath2.snapshotLength-2).cloneNode(true);
		xpath = document.evaluate(".//div[@id='posts']/div"
			,obj,null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
		for (var i=xpath.snapshotLength-2;i>=0 ;i-- )
		{
			content.splice(1, 0, xpath.snapshotItem(i).cloneNode(true));
		}
		content.forEach(function(item){
			protected.insertPoint.parentNode.insertBefore(item,protected.insertPoint);
		});
...
var IPB3 = function(){
	...
	that.init = function(){
		protected.infoBox.init();
    var max_page=-1;
		var xpath=document.evaluate(".//li[@class='total']/text()"
			,document,null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);      
		if (xpath.snapshotLength===2)
    {
    ...
    var cur_page=protected.maxPage;
		xpath=document.evaluate(".//li[@class='active']/text()"
			,document,null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);    
    ...
    protected.url = a.toString().replace(/&st=\d+/,'&st='+((protected.nextPage-1)*protected.st));
		...
		xpath=document.evaluate("./html/body//div[@id='content']/*[last()]"
			,document,null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
		protected.insertPoint=xpath.snapshotItem(0);
	};
	that.ProcessResultData = function(data, obj){
    var content=[];
		var xpath=document.evaluate(".//div[contains(concat(' ',normalize-space(@class),' '),' topic ')]"
			,obj,null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);      
...
var IPB2 = function(){
	...
	that.init = function(){
		protected.infoBox.init();
    var max_page=-1;
		var xpath=document.evaluate(".//span[contains(concat(' ',normalize-space(@class),' '),' pagelink ')]/text()"
        ,document,null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);      
    if (xpath.snapshotLength===2)
    {
      var regex_result=xpath.snapshotItem(0).nodeValue.match(/(\d+)/);
    ...
    var cur_page=protected.maxPage;
		xpath=document.evaluate(".//span[contains(concat(' ',normalize-space(@class),' '),' pagecurrent ')]/text()"
			,document,null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
    ...
    protected.url = a.toString().replace(/&st=\d+/,'&st='+((protected.nextPage-1)*protected.st));
		...
		xpath=document.evaluate("./html/body//div[contains(concat(' ',normalize-space(@class),' '),' borderwrap ')]/div[contains(concat(' ',normalize-space(@class),' '),' maintitle ')]/table/../../following-sibling::*[1]"
			,document,null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
	...
	that.ProcessResultData = function(data, obj){
    var content=[];
		var xpath=document.evaluate(".//div[contains(concat(' ',normalize-space(@class),' '),' borderwrap ')]/div[contains(concat(' ',normalize-space(@class),' '),' maintitle ')]/table/../.."
			,obj,null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);      
    if (xpath.snapshotLength===1)
      content[0]=xpath.snapshotItem(0).cloneNode(true);
    else
...
var IPB1 = function(){
	...
	that.init = function(){
		protected.infoBox.init();
		var xpath=document.evaluate(".//a[@name='top']/following-sibling::table[1]/tbody/tr/td[1]"
			,document,null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);      
    var max_page=-1;    
		if (xpath.snapshotLength===1)
    {
    ...
    protected.url = a.toString().replace(/&st=\d+/,'&st='+((protected.nextPage-1)*protected.st));
		...
		xpath=document.evaluate("./html/body/table[last()-1]"
			,document,null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
		var page_bar=xpath.snapshotItem(0);
		page_bar.setAttribute('style', 'visibility: hidden');

		xpath=document.evaluate("./html/body/table[last()-2]/tbody/tr[last()]"
			,document,null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
		protected.insertPoint=xpath.snapshotItem(0);
	};
	that.ProcessResultData = function(data, obj){
    var content=[];
		var xpath=document.evaluate(".//a[@name='top']/following-sibling::table[1]/tbody/tr"
			,obj,null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);      
    ...
    content[0]=table;
		...
		xpath=document.evaluate(".//table[last()-2]/tbody/tr[last()-1]"
			,obj,null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
    content[1]=xpath.snapshotItem(0).cloneNode(true);
...
if (generator!=null){
	...
	if (generator.match("Discuz! 6") || generator.match("Discuz  7.0.0")){
